...
const STATUS_NOTA = {
  nao_emitida: { label: "Nota fiscal pendente", cor: MUTED },
  emitida_nao_paga: { label: "Emitida · aguardando pagamento", cor: AMARELO },
  paga: { label: "Liquidada", cor: VERDE },
};
    ...
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", color: INK, minHeight: "100vh", display: "flex",
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        <main style={{ padding: 28, flex: 1 }}>
          ...
          {aba === "agenda" && (
            <Secao titulo="Agenda de serviços" onAdd={() => setModal({ tipo: "servico", data: { data: hoje(), status: "nao_emitida", condId: db.condominios[0]?.id, valor: 0 } })}>
              <table><thead><tr><th>Data</th><th>Serviço</th><th>Condomínio</th><th>Valor</th><th>Situação fiscal</th><th></th></tr></thead>
                <tbody>{[...servicos].sort((a, b) => a.data.localeCompare(b.data)).map((s) => (
                  <tr key={s.id}>
                    <td style={{ fontWeight: diasAte(s.data) >= 0 && diasAte(s.data) <= 7 ? 800 : 400, color: diasAte(s.data) >= 0 && diasAte(s.data) <= 7 ? NAVY : INK }}>{fmtData(s.data)}</td>
                    <td>{s.titulo}</td><td>{nomeCliente(s)}</td><td>{brl(s.valor)}</td>
                  ...
                  {servicos.length === 0 && <tr><td colSpan={6} style={{ color: MUTED }}>Nenhum serviço agendado no momento.</td></tr>}
                </tbody></table>
            </Secao>
          )}
          ...
          {aba === "financeiro" && (
            <div className="fade">
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 18, marginBottom: 22 }}>
                <KPI titulo="Valores a faturar" valor={brl(naoEmitido)} cor={LARANJA} sub="emissão de nota fiscal pendente" />
                <KPI titulo="Valores a receber" valor={brl(aReceber)} cor={AMARELO} sub="aguardando pagamento" />
                <KPI titulo="Total recebido" valor={brl(recebido)} cor={VERDE} sub="notas fiscais liquidadas" />
              </div>
              <div className="glass" style={{ overflow: "hidden" }}>
                <div style={{ padding: "16px 18px", fontWeight: 800, color: TITULO, borderBottom: `1px solid ${LINE}` }}>
                  Controle de notas fiscais · selecione o status para avançar (pendente → emitida → liquidada)
                </div>
                <table><thead><tr><th>Serviço</th><th>Condomínio</th><th>Valor</th><th>NF nº</th><th>Pago em</th><th>Status</th></tr></thead>
              ...
              <div style={{ marginTop: 22 }} className="glass">
                <div style={{ padding: "16px 18px", fontWeight: 800, color: TITULO, borderBottom: `1px solid ${LINE}` }}>
                  Conciliação · serviços executados × notas fiscais
                </div>
                <table><thead><tr><th>Serviço</th><th>Condomínio</th><th>Valor do serviço</th><th>Nota fiscal</th><th>Conferência</th></tr></thead>
                  <tbody>{[...db.servicos].sort((a, b) => b.data.localeCompare(a.data)).map((s) => {
                    ...
                    return (
                      <tr key={s.id}>
                        <td>{s.titulo}</td>
                        <td>{nomeCliente(s)}</td>
                        <td style={{ fontWeight: 600 }}>{brl(s.valor)}</td>
                        <td>{temNota ? `NF ${s.nfNumero || "—"}` : <span style={{ color: MUTED }}>não emitida</span>}</td>
                        <td>
                          {temNota
                            ? <Badge cor={VERDE}>Nota emitida</Badge>
                            : <Badge cor={LARANJA}>Faturar</Badge>}
                        </td>
                      </tr>
                    );
                  })}</tbody></table>
                <div style={{ padding: "12px 18px", fontSize: 13, color: MUTED, borderTop: `1px solid ${LINE}`, display: "flex", gap: 24, flexWrap: "wrap" }}>
                  <span>Serviços executados: <strong style={{ color: TITULO }}>{db.servicos.length}</strong></span>
                  <span>Com nota emitida: <strong style={{ color: VERDE }}>{db.servicos.filter((s) => s.status !== "nao_emitida").length}</strong></span>
                  <span>Pendentes de nota: <strong style={{ color: LARANJA }}>{db.servicos.filter((s) => s.status === "nao_emitida").length}</strong></span>
                  <span>Valor total executado: <strong style={{ color: TITULO }}>{brl(db.servicos.reduce((t, s) => t + s.valor, 0))}</strong></span>
                </div>
              </div>
            </div>
          )}
  ...
  return (
    <div className="fade">
      ...
      {chaves.map((chave) => {
        ...
        const total = lista.reduce((t, s) => t + s.valor, 0);
        const faturado = lista.filter((s) => s.status !== "nao_emitida").reduce((t, s) => t + s.valor, 0);
        return (
          <div key={chave} className="glass" style={{ overflow: "hidden", marginBottom: 18 }}>
            <div style={{ padding: "14px 18px", borderBottom: `1px solid ${LINE}`, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
              ...
              <div style={{ marginLeft: "auto", display: "flex", gap: 22, fontSize: 13 }}>
                ...
                <span style={{ color: MUTED }}>Faturado: <strong style={{ color: VERDE }}>{brl(faturado)}</strong></span>
              </div>
            </div>
            <table><thead><tr><th>Data</th><th>Serviço</th><th>Condomínio</th><th>Valor</th><th>Situação fiscal</th></tr></thead>
              <tbody>{lista.map((s) => (
                <tr key={s.id}>
...
function FormServico({ data, conds, onSave, onClose }) {
  const [f, setF] = useState({ titulo: "", data: hoje(), valor: 0, status: "nao_emitida", nfNumero: "", pgtoData: "", clienteAvulso: "", condId: conds[0]?.id, ...data });
  const ehAvulso = f.condId === AVULSO;
  return (
    <Modal titulo={data.id ? "Editar serviço" : "Cadastrar serviço"} onClose={onClose}>
      ...
      <label>Valor (R$)</label><input type="number" value={f.valor} onChange={(e) => setF({ ...f, valor: parseFloat(e.target.value) || 0 })} />
